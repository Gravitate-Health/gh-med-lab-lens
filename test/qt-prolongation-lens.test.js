...
let annotation;
beforeAll(() => {
  const scriptContent = fs.readFileSync(path.join(__dirname, "../qt-prolongation-lens.js"), "utf-8");
...
describe("QT Prolongation Annotation Script (non-invasive)", () => {
  ...
  test("should return enhanced HTML containing highlight class", async () => {
    ...
    expect(result).toContain("qt-prolongation-lens");
  });
});
